...
function Story({ uploader, accessControl, title, isUser, user1Images }: StoryProps) {
  return (
    <div>
      <ul className="flex gap-10 p-0 m-3 list-none">
        {isUser
          ? user1Images.map((imageUrl) => (
              <div key={imageUrl} className="relative shadow-ButtonShadow">
                <img src={imageUrl} alt="User1's Image" className="object-cover w-48 h-48 mt-0" />
                <div className="absolute top-[-1.2rem] w-32 left-8">
